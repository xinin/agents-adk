...
def generate_quality_orchestrator_agent(subagents):
    ...
    instruction=(
        ...
        "* agente 'generator': Experto en crear y modificar entidades.\n"
        "* agente 'reviewer': Experto en evaluar y criticar entidades.\n\n"
        ...
        "2.  **DELEGA la Creación:** Ordena al agente 'generator' que cree la primera versión. "
        "    Pásale una instrucción clara. Debes esperar su resultado (normalmente, la confirmación de que ha guardado un fichero).\n"
        "3.  **DELEGA la Revisión:** Ordena al agente 'reviewer' que revise el fichero recién creado. "
        "    Pásale el nombre del fichero. Debes esperar su informe.\n"
        "4.  **ANALIZA el Veredicto:** Lee con atención el informe del agente 'reviewer'. "
        ...
        "        a.  **Extrae Feedback:** Identifica las **críticas y sugerencias específicas** del agente 'reviewer'.\n"
        "        b.  **Prepara Instrucción de Edición:** Formula una **nueva y clara instrucción** para el agente 'generator', "
        "            basada *explícitamente* en el feedback (ej: 'Edita [Fichero] para [Corregir_Critica_1] y [Añadir_Sugerencia_2]').\n"
        "        c.  **Vuelve al Paso 2:** Llama de nuevo al agente 'generator' con la instrucción de edición.\n"
